...
学校：上海大学 信息管理 本科
求职意向：项目协调实习
技能：Sla ck、Asa na、Trel lo、No tion
项目经历：使用 Sla ck 协调跨组排期，在 Asa na 和 Trel lo 跟进任务拆解，并通过 No tion 维护会议纪要。`);
  const job = analyzeJobDescription({
    title: '项目协调实习生',
    city: '上海',
    description: '需要熟悉 Slack、Asana、Trello、Notion，能支持跨团队沟通、任务推进和文档沉淀。',
  });
...
test('filters the HR review queue by search text and review stage', () => {
  ...
  const nativePdfEnglishQueryIds = filterHrCandidatesForReview(candidates, JOBS, { query: 'Native PDF Text' }).map((candidate) => candidate.id);
  ...
  assert.deepEqual(
    filterHrCandidatesForReview(candidates, JOBS, { query: 'OpenAI OCR Text' }).map((candidate) => candidate.id),
    [openAiOcrCandidate.id],
  );
  assert.deepEqual(
  ...
  assert.deepEqual(
    filterHrCandidatesForReview(candidates, JOBS, { query: 'manual review' }).map((candidate) => candidate.id),
    [uploadOnlyCandidate.id],
  );
  assert.deepEqual(
  ...
  assert.deepEqual(
    filterHrCandidatesForReview(candidates, JOBS, { query: '人工审核' }).map((candidate) => candidate.id),
    [uploadOnlyCandidate.id],
  );
  assert.deepEqual(
    filterHrCandidatesForReview(candidates, JOBS, { query: '待人工复核' }).map((candidate) => candidate.id),
    [uploadOnlyCandidate.id],
  );
  assert.deepEqual(
  ...
  assert.deepEqual(
    filterHrCandidatesForReview(candidates, JOBS, { query: '需复核' }).map((candidate) => candidate.id),
    [uploadOnlyCandidate.id],
  );
  assert.ok(
  ...
  assert.deepEqual(
    filterHrCandidatesForReview(candidates, JOBS, { stage: 'Native PDF Text' }).map((candidate) => candidate.id),
    nativePdfStageIds,
  );
  assert.deepEqual(
  ...
  assert.deepEqual(
    filterHrCandidatesForReview(candidates, JOBS, { stage: 'manual-review' }).map((candidate) => candidate.id),
    [uploadOnlyCandidate.id],
  );
  assert.deepEqual(
  ...
  assert.deepEqual(
    filterHrCandidatesForReview(candidates, JOBS, { stage: '需复核' }).map((candidate) => candidate.id),
    [uploadOnlyCandidate.id],
  );
});
