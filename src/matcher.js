...
const KEYWORD_ALIASES = {
  ...
  Semrush: ['semrush', 'SEMrush', 'Sem Rush', 'Sem rush', 'sem rush', 'SEM rush', 'seo关键词工具'],
  ...
  Vercel: ['vercel'],
  Netlify: ['netlify', 'Net li fy', 'net li fy'],
  ...
  Mailchimp: ['mailchimp', 'Mail Chimp', 'mail chimp'],
  Klaviyo: ['klaviyo', 'Kla viyo', 'kla viyo'],
  Braze: ['braze', 'Bra ze', 'bra ze'],
  Lovable: ['lovable', 'Loveable', 'loveable'],
  Base44: ['base44', 'Base 44', 'base 44'],
  Replit: ['replit', 'Re plit', 're plit'],
  Remotion: ['remotion', 'Re motion', 're motion'],
  Canva: ['canva', 'Can va', 'can va'],
  Figma: ['figma', 'Fi gma', 'fi gma'],
  Wix: ['wix', 'W ix', 'w ix'],
  HeyGen: ['heygen', 'Hey gen', 'hey gen', 'Hey Gen'],
  Zotero: ['zotero', 'Zo tero', 'zo tero'],
  Cloudflare: ['cloudflare', 'Cloud flare', 'cloud flare'],
  'Cloudflare Workers': ['cloudflare workers', 'Cloud flare Workers', 'cloud flare workers'],
  'Cloudflare Pages': ['cloudflare pages', 'Cloud flare Pages', 'cloud flare pages'],
  Turnstile: ['turnstile', 'Turn stile', 'turn stile'],
  Linear: ['linear', 'Li near', 'li near'],
  Playwright: ['playwright', 'Play wright', 'play wright'],
  Selenium: ['selenium', 'Sele nium', 'sele nium'],
  Docker: ['docker', 'Dock er', 'dock er'],
  Kubernetes: ['kubernetes', 'K8s', 'k8s', 'Kuber netes', 'kuber netes'],
  Postman: ['postman', 'Post man', 'post man'],
  Framer: ['framer', 'Fra mer', 'fra mer'],
  Webflow: ['webflow', 'Web flow', 'web flow'],
  Notion: ['notion', 'No tion', 'no tion'],
  Jira: ['JIRA', 'jira', 'Jira Software', 'jira software', 'Ji ra', 'ji ra'],
  Slack: ['slack', 'S lack', 's lack', 'Sla ck', 'sla ck'],
  Lark: ['lark', '飞书'],
  ...
  Box: ['box', 'Box Drive', 'box drive'],
  Dropbox: ['dropbox', 'Drop box', 'drop box'],
...
function normalizeSalaryText(value) {
  return String(value ?? '')
    ...
    .replace(/\s*[，,;；]\s*(?:另有)?\s*(\d{1,2}\s*薪)/g, '·$1')
    .replace(/((?:元\/天|元\/日|元\/月|\/天|\/月|\/年))\s+(\d{1,2}\s*薪)/g, '$1·$2')
    .replace(/(\d+(?:\.\d+)?)\s*([Kk千]|万)\s*([~至–—-])\s*(\d+(?:\.\d+)?)\s*\2(?=(?:\/月|\/年|$))/g, '$1$2$3$4$2')
    .replace(/[~至–—]/g, '-')
    .replace(/\s*(-)\s*/g, '$1')
    .replace(/\s+/g, '')
    .trim();
...
export function parseJobDescription(description) {
  const salaryMatch = description.match(
    /(?:薪资|薪酬|月薪|日薪|综合薪资|薪资待遇)[：:\s]*([￥¥]?\s*[0-9]+(?:\.\d+)?(?:\s*(?:[Kk千]|万))?(?:\s*[-~至–—]\s*[￥¥]?\s*[0-9]+(?:\.\d+)?(?:\s*(?:[Kk千]|万))?)?\s*(?:元\/天|元\/日|元\/月|\/天|K\/月|k\/月|千\/月|万\/月|万\/年|[Kk千万])(?:\s*(?:(?:[·•]|[×xX*])\s*|[，,;；]\s*(?:另有)?\s*|\s+)\d{1,2}\s*薪)?)/,
  );
...
function getHrCandidateExtractionSearchTerms(candidate) {
  ...
  const labels = [
    ...
    textSource === 'pdf-text' ? 'Native PDF Resume' : '',
    textSource === 'pdf-text' ? 'PDF Text Extraction' : '',
    textSource === 'pdf-text' ? '原生 PDF 提取' : '',
    textSource === 'pdf-text' ? '原生 PDF 文本提取' : '',
    textSource === 'openai-ocr' ? 'OpenAI OCR' : '',
    textSource === 'openai-ocr' ? 'OpenAI OCR Text' : '',
    textSource === 'openai-ocr' ? 'AI OCR 提取' : '',
    textSource === 'openai-ocr' ? 'OCR Extraction' : '',
    textSource === 'pdf-text-fallback' ? 'OCR Fallback' : '',
    textSource === 'pdf-text-fallback' ? 'OCR 保底' : '',
    textSource === 'pdf-text-fallback' ? 'PDF 保底提取' : '',
    textSource === 'pdf-text-fallback' ? 'ocr-fallback' : '',
    textSource === 'pdf-text-fallback' ? 'OCR Warning' : '',
    textSource === 'pdf-text-fallback' ? 'manual review' : '',
    textSource === 'pdf-text-fallback' ? 'manual screening' : '',
    textSource === 'pdf-text-fallback' ? 'needs review' : '',
    textSource === 'pdf-text-fallback' ? 'review required' : '',
    textSource === 'pdf-text-fallback' ? '人工复核' : '',
    textSource === 'pdf-text-fallback' ? '人工审核' : '',
    textSource === 'pdf-text-fallback' ? 'OCR 复核' : '',
    textSource === 'pdf-text-fallback' ? '待人工复核' : '',
...
function normalizeHrCandidateStage(stage) {
  ...
  const aliasMap = {
    ...
    'high-potential-unapplied': 'high-potential-unsubmitted',
    '高潜待分流': 'high-potential-unsubmitted',
    '高潜未投递': 'high-potential-unsubmitted',
    ...
    'native-pdf-extraction': 'native-pdf',
    ...
    'ai-ocr': 'openai-ocr',
    'ai-ocr-提取': 'openai-ocr',
    'openai-ocr-extraction': 'openai-ocr',
    'ocr-extraction': 'openai-ocr',
    ocr: 'openai-ocr',
    ...
    'manual-review-queue': 'ocr-fallback',
    'manual-screening': 'ocr-fallback',
    'review-required': 'ocr-fallback',
    '人工审核': 'ocr-fallback',
    '待人工复核': 'ocr-fallback',
