...
function isPageMarkerLine(line) {
  ...
  return [
    ...
    /^第\s*\d+\s*页\s*[,，]\s*共?\s*\d+\s*页?$/i,
    /^页码\s*[:：]?\s*\d+(?:\s*[\/／]\s*\d+)?$/i,
    /^page\s*(?:no\.?\s*)?\d+(?:\s*(?:of|sur|\/)\s*\d+)?$/i,
    /^page\s*\d+(?:\s*(?:of|sur|\/)\s*\d+)?$/i,
    /^page\s*\d+\s*[,，-]\s*(?:of\s*)?\d+$/i,
    /^pg\.?\s*\d+(?:\s*(?:of|sur|\/)\s*\d+)?$/i,
    /^p(?:age)?[.:]?\s*\d+(?:\s*[\/／]\s*\d+)?$/i,
    /^\d+\s*[\/／]\s*\d+$/,
    /^[\-–—_~·•]+\s*\d+\s*[\-–—_~·•]+$/,
...
export function shouldUseOcrTextExtraction(text) {
  ...
  const hasConciseStructuredHeader =
    ...
    lineCount >= 3 &&
    compact.length >= 32 &&
    /(姓名|邮箱|电子邮箱|电话|手机号|联系电话|联系方式|微信|\bName\b|\bAddress\b|\bCurrent City\b|\bCurrent Location\b|\bCurrent Address\b|\bPreferred City\b|\bPreferred Location\b|\bPresent Location\b|\bCurrent Employer\b|\bDate of Birth\b|\bE-?mail(?: Address)?\b|\bEmail ID\b|\bMail ID\b|\bPhone(?: Number| No\.?)?\b|\bContact Number\b|\bMobile\b|\bCell\b|\bTel\b|\bTelephone\b|\bWeChat\b|\bWhatsApp\b|\bContact(?: Information| Details)?\b|\bContact\b|\bLinkedIn\b|\bPortfolio\b|\bWebsite\b|\bPersonal (?:Information|Details)\b|\bProfile Details\b|\bProfile Summary\b)/i.test(normalized) &&
    /(技能|项目|实习|教育|求职意向|\bSkill Set\b|\bSkills?(?: & Tools)?\b|\bTechnical Skills\b|\bTechnical Proficiencies\b|\bProjects?\b|\bExperience\b|\bEducation(?: Background)?\b|\bObjective\b|\bTarget Role\b|\bTools\b|\bLanguages?\b|\bAvailability(?: to Join)?\b|\bAvailable to Join\b|\bCurrent Availability\b|\bAvailable From\b|\bAvailable to Start\b|\bCurrent CTC\b|\bExpected CTC\b|\bCurrent Salary\b)/i.test(normalized);
  const hasConciseContactLinkHeader =
    identityLabelMatches.length >= 3 &&
    lineCount >= 3 &&
    compact.length >= 24 &&
    /(?:\bName\b|\bAddress\b|\bCurrent Address\b|\bE-?mail(?: Address)?\b|\bEmail ID\b|\bMail ID\b|\bPhone(?: Number| No\.?)?\b|\bContact Number\b|\bMobile\b|\bCell\b|\bTel\b|\bTelephone\b|\bContact(?: Information| Details)?\b|\bPersonal (?:Information|Details)\b|\bProfile Details\b)/i.test(normalized) &&
