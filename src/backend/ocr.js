...
export function shouldUseOcrTextExtraction(text) {
  ...
  const hasConciseStructuredHeader =
    ...
    lineCount >= 3 &&
    compact.length >= 32 &&
    /(姓名|邮箱|电子邮箱|电话|手机号|联系电话|联系方式|微信|\bName\b|\bAddress\b|\bCurrent City\b|\bPreferred City\b|\bPresent Location\b|\bDate of Birth\b|\bE-?mail(?: Address)?\b|\bPhone(?: Number)?\b|\bMobile\b|\bCell\b|\bTel\b|\bTelephone\b|\bWeChat\b|\bContact(?: Information| Details)?\b|\bContact\b|\bLinkedIn\b|\bPortfolio\b|\bWebsite\b|\bPersonal (?:Information|Details)\b|\bProfile Details\b|\bProfile Summary\b)/i.test(normalized) &&
    /(技能|项目|实习|教育|求职意向|\bSkill Set\b|\bSkills?(?: & Tools)?\b|\bTechnical Skills\b|\bTechnical Proficiencies\b|\bProjects?\b|\bExperience\b|\bEducation(?: Background)?\b|\bObjective\b|\bTarget Role\b|\bTools\b|\bLanguages?\b|\bAvailability(?: to Join)?\b|\bCurrent Availability\b|\bAvailable From\b|\bAvailable to Start\b)/i.test(normalized);
  const hasConciseContactLinkHeader =
    identityLabelMatches.length >= 3 &&
    lineCount >= 3 &&
    compact.length >= 24 &&
    /(?:\bName\b|\bAddress\b|\bE-?mail(?: Address)?\b|\bPhone(?: Number)?\b|\bMobile\b|\bCell\b|\bTel\b|\bTelephone\b|\bContact(?: Information| Details)?\b|\bPersonal (?:Information|Details)\b|\bProfile Details\b)/i.test(normalized) &&
