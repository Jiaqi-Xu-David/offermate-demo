...
export function shouldUseOcrTextExtraction(text) {
  ...
  const hasConciseStructuredHeader =
    ...
    lineCount >= 3 &&
    compact.length >= 32 &&
    /(姓名|邮箱|电子邮箱|电话|手机号|联系电话|联系方式|微信|\bName\b|\bAddress\b|\bDate of Birth\b|\bE-?mail(?: Address)?\b|\bPhone(?: Number)?\b|\bMobile\b|\bCell\b|\bTel\b|\bTelephone\b|\bWeChat\b|\bContact(?: Information| Details)?\b|\bLinkedIn\b|\bPortfolio\b|\bWebsite\b|\bPersonal (?:Information|Details)\b|\bProfile Details\b)/i.test(normalized) &&
    /(技能|项目|实习|教育|求职意向|\bSkill Set\b|\bSkills?(?: & Tools)?\b|\bTechnical Skills\b|\bTechnical Proficiencies\b|\bProjects?\b|\bExperience\b|\bEducation(?: Background)?\b|\bObjective\b|\bTarget Role\b|\bLanguages?\b|\bAvailability(?: to Join)?\b)/i.test(normalized);
